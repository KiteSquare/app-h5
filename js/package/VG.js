...
(function($) {
    ...
    od.VG = new function() {
        ...
        this.method = function(webview, methodName, param, callback) {
            ...
            evalJS(webview, "od.VG._methodCalled", curViewId, methodName, JSON.stringify(param), callbackKey);
        }

        this._methodCalled = function(viewId, methodName, param, callbackKey) {
            //TODO 多参数如何实现
            var mns=methodName.split(".");
            var method=window;;
            for(var i in mns){
            	method=method[mns[i]];
            }
            method(JSON.parse(param), callbackKey && function(backParam) {
                evalJS(plus.webview.getWebviewById(viewId), "od.VG._methodBacked", callbackKey, backParam && JSON.stringify(backParam) || "");
            });
        }
